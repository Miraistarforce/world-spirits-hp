...
(function() {
  ...
  function applyMessage(c) {
    ...
    if (c.visionText) setText('.p-home-vision__text', c.visionText);
    if (c.bgText) setText('.p-service-show__bg', c.bgText);
    ...
    if (c.items && c.items.length) {
      ...
      c.items.forEach(function(item, i) {
        ...
        if (faces[i] && item.image) faces[i].src = item.image;
        if (spCards[i]) {
    ...
    if (c.years) {
      ...
      c.years.forEach(function(yearData) {
        ...
        (yearData.quarters || []).forEach(function(q) {
          ...
          cardsHtml += '<div class="p-sales__card"><button class="p-sales__card-header" type="button" aria-expanded="false"><span class="p-sales__card-label">' + escHtml(q.label) + '</span><span class="p-sales__card-icon"></span></button><div class="p-sales__card-body"><div class="p-sales__card-body-inner"><table class="p-sales__table"><thead><tr><th>月</th><th>催事店舗名</th><th class="p-sales__th-amount">売上金額</th></tr></thead><tbody>' + rowsHtml + '</tbody></table></div></div></div>';
        });
        yearDiv.innerHTML = '<h3 class="p-sales__year-title">' + escHtml(yearData.year) + '年</h3><div class="p-sales__cards">' + cardsHtml + '</div>';
  ...
  function applyHomeNews(c) {
    ...
    c.items.slice(0, 3).forEach(function(item) {
      ...
      li.innerHTML = '<a class="c-news-list__link" href="news.html"><div class="c-news-list__head"><time class="c-news-list__date">' + escHtml(item.date) + '</time><span class="c-news-list__category">' + escHtml(item.category) + '</span></div><div class="c-news-list__body"><h3 class="c-news-list__title">' + escHtml(item.title) + '</h3><span class="c-news-list__dot"></span></div></a>';
      list.appendChild(li);
    });
  }
  ...
  function updateMenus(content) {
    Object.keys(MENU_MAP).forEach(function(sectionId) {
      ...
      document.querySelectorAll('.g-header-nav__link').forEach(function(link) {
        if (link.getAttribute('href') === href) link.textContent = c.menuTitle;
      });
